...
所有公開 MCP 工具都以自然語言 `query` 作為唯一公司識別來源：
...
| 欄位 | 型別 | 必填 | 說明 |
|---|---|---:|---|
| `query` | string | 是 | 自足的使用者問題；不得只傳關鍵字集合 |

規則：

- `query` 是唯一公司判斷依據；公開 Tool 不接受 `co_code`。
- 對話追問由呼叫端補成包含既有公司脈絡的自足 query。
