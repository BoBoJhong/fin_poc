...
Agent 必須保留不同 MCP 的來源與信任等級，不得把另一個 MCP 的內容標示成本專案已回答。
若要合成答案，分別保留 citation、status、warning 與資料版本。
...
2. 決定沿用 `VerifiedRAGResponse 2.0`，或建立新的 major schema。
...
只新增內部 Provider 且公開 Envelope／語意不變，可維持 Schema `2.0`，但必須更新
coverage 與 Provider 文件。新增公開 Tool 是 additive change；移除／改名 Tool、
